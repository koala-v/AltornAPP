    ...
    function (
        ...
        var blnVerifyInput = function (type) {
            ...
            if (is.equal(type, 'StoreNo') && is.not.equal($scope.Detail.Scan.StoreNo, $scope.Detail.Imgi2.StoreNo)) {
                ...
                var SerialNoSplit = $scope.Detail.Imgi2.SerialNo.split(',');
                for (var intI = 0; intI < SerialNoSplit.length; intI++) {
                    hmSeialNo.set(SerialNoSplit[intI], SerialNoSplit[intI]);
                }
                if (hmSeialNo.count() > 0) {
                    if (hmSeialNo.has($scope.Detail.Scan.SerialNo)) {
                        return blnPass;
                    } else {
                        blnPass = false;
                        PopupService.Alert(popup, 'Invalid Product Picked').then();
        ...
        $scope.openCam = function (type) {
            if (!ENV.fromWeb) {
                if (is.equal(type, 'StoreNo')) {
                    ...
                    $cordovaBarcodeScanner.scan().then(function (imageData) {
                        $scope.Detail.Scan.UserDefine1 = imageData.text;
                        if (blnVerifyInput('UserDefine1')) {
                ...
                } else if (is.equal(type, 'DeliveryToCode')) {
                    $cordovaBarcodeScanner.scan().then(function (imageData) {
                        $scope.Detail.Scan.DeliveryToCode = imageData.text;
                        if (blnVerifyInput('DeliveryToCode')) {
        ...
        $scope.clearInput = function (type) {
            if (is.equal(type, 'BarCode')) {
                ...
                if ($scope.Detail.Scan.DeliveryToCode.length > 0) {
                    $scope.Detail.Scan.DeliveryToCode = '';
                    $('#txt-DeliveryToCode').select();
                }

            } else if (is.equal(type, 'UserDefine1')) {
                if ($scope.Detail.Scan.UserDefine1.length > 0) {
                    $scope.Detail.Scan.UserDefine1 = '';
                    $('#txt-UserDefine1').select();
                }
        ...
        var showScanCustBatchNo = function (ScanCustBatchNo) {
            if (is.not.empty(ScanCustBatchNo)) {
                ...
                if (SplitScanCustBatchNo.length > 0) {
                    ...
                    if (is.equal($scope.Detail.Imgr2.ScanCustBatchNo, $scope.Detail.Imgr2.CustBatchNo)) {
